...
class ScreenshotThread(
    ...
    override fun run() {
        try {
            ...
            if (NicephoreConfigHolder.current.makeJPEGs) {
                val writer = ImageIO.getImageWritersByFormatName("jpg").next()
                val params = writer.defaultWriteParam
                params.compressionMode = ImageWriteParam.MODE_EXPLICIT
                params.progressiveMode = ImageWriteParam.MODE_DEFAULT
                params.compressionQuality = NicephoreConfigHolder.current.compression.toFloat()
            ...
            if (NicephoreConfigHolder.current.optimiseScreenshots) {
                val shouldShowOptStatus = NicephoreConfigHolder.current.showOptimisationStatus
                ...
                try {
                    ...
                    val cmd = MessageFormat.format(Reference.Command.OXIPNG, oxipng, NicephoreConfigHolder.current.pngOptimisationLevel, pngFile)
                    ProcessBuilder(cmd.split(" ")).start().waitFor()
                } catch (e: Exception) {
            ...
            if (NicephoreConfigHolder.current.screenshotCustomMessage) {
                if (NicephoreConfigHolder.current.screenshotToClipboard) {
